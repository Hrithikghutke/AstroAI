...
TOKEN BUDGET:
- Hard limit: TARGET_TOKENS tokens
...
<section class="relative min-h-screen flex items-center justify-center text-center overflow-hidden" style="background:var(--b1)">
  <!-- Glowing orbs -->
  <div class="absolute top-1/4 left-1/4 w-125 h-125 rounded-full blur-[120px] opacity-25 pointer-events-none" style="background:var(--p)"></div>
  <div class="absolute bottom-1/4 right-1/4 w-100 h-100 rounded-full blur-[100px] opacity-15 pointer-events-none" style="background:var(--s)"></div>
  <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-75 h-75 rounded-full blur-[80px] opacity-10 pointer-events-none" style="background:var(--p)"></div>
  <!-- Dot grid pattern -->
...
- Only <style> allowed: @keyframes if needed
- Write routing JS FIRST in script block — non-negotiable
- ALWAYS end with </script></body></html> — never leave HTML unclosed
- Prioritise closing the document over adding more content`;
}
